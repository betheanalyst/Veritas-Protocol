...
export function mapTask(raw: Raw): Task {
  ...
  return {
    ...
    // get_task_batch omits module_type, metadata, max_dispute_rounds,
    // challenge_window_sec, and reasoning — use safe defaults so the list
    // renders without crashing (the full get_task view provides them all).
    moduleType: typeof raw.module_type === "string" ? raw.module_type : "",
    ...
    maxDisputeRounds: raw.max_dispute_rounds !== undefined ? toCount(raw.max_dispute_rounds, "max_dispute_rounds") : 0,
    challengeWindowSec: raw.challenge_window_sec !== undefined ? toCount(raw.challenge_window_sec, "challenge_window_sec") : 0,
